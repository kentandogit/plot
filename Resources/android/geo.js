...
var GeoData = function(title, latitude, longitude, parkinglot) {
    this.title = title;
    this.coords = {
        latitude: latitude,
        longitude: longitude
...
exports.createGeoData = function(parkingl, callback) {
    callback(new GeoData(parkingl.get("name"), parkingl.get("lat"), parkingl.get("long"), parkingl));
};
...
var forwardGeocodeNative = function(address, callback) {
    ...
    xhr.onload = function() {
        ...
        callback(new GeoData(address, json.results[0].geometry.location.lat, json.results[0].geometry.location.lng, null));
    };
...
var forwardGeocodeWeb = function(address, callback) {
    ...
    geocoder ? geocoder.geocode({
        ...
        if (status == google.maps.GeocoderStatus.OK) callback(new GeoData(address, results[0].geometry.location.lat(), results[0].geometry.location.lng(), null)); else {
            Ti.API.error(status);
            alert(ERROR_MESSAGE);
        }
